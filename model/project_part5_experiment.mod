...
# (1) change the parameter downtime to some value <= 34 and the parameter faa_regulation_time to some value >=750
# (2) define parameter n of your choice which can range [0,16] and replace all constraints under "# added" with
# constraints below without changing any other parameters:
...
# return_number_cap: sum {(f,c) in VALID_CAPTAIN_FINAL} L_cap[f,c] >= n;
# return_number_fo: sum {(f,fo) in VALID_FO_FINAL} L_fo[f,fo] >= n;
...
param downtime := 34;  # modified
param faa_regulation_time := 750;  # modified
